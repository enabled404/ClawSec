...
fn parse_args(args: &[String]) -> Result<CliAction, String> {
    ...
    while index < args.len() {
        match args[index].as_str() {
            ...
            "-p" => {
                // ClawSec compat: -p "prompt" = one-shot prompt
            ...
            "--print" => {
                // ClawSec compat: --print makes output non-interactive
...
impl LiveCli {
    ...
    fn startup_banner(&self) -> String {
        ...
        let has_claw_md = cwd
            .as_ref()
            .is_some_and(|path| path.join("CLAWSEC.md").is_file() || path.join("CLAW.md").is_file());
        let mut lines = vec![
            format!(
                "{} {}",
                if color {
                    "\x1b[1;38;5;196m🦀 Claw Security\x1b[0m"
                } else {
                    "Claw Security"
                },
                if color {
        ...
        if !has_claw_md {
            lines.push(
                "  First run        /init scaffolds CLAWSEC.md, .claw.json, and local session files"
                    .to_string(),
            );
        }
        lines.join("\n")
...
fn print_help_to(out: &mut impl Write) -> io::Result<()> {
    writeln!(out, "Claw Security CLI v{VERSION}")?;
    writeln!(
        out,
        "  Autonomous penetration testing and vulnerability scanning agent."
    )?;
    writeln!(out)?;
    ...
    writeln!(
        out,
        "  claw init                             Scaffold CLAWSEC.md + local files"
    )?;
    writeln!(out)?;
    writeln!(out, "Flags")?;
    writeln!(
...
mod tests {
 ...
 M src/main.rs",
    ...
    fn init_template_mentions_detected_rust_workspace() {
        ...
        assert!(rendered.contains("# CLAWSEC.md"));
