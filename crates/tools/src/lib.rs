...
pub fn mvp_tool_specs() -> Vec<ToolSpec> {
    vec![
        ...
        ToolSpec {
            name: "Config",
            description: "Get or set Claw Security settings.",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "setting": { "type": "string" },
        ...
        ToolSpec {
            name: "RunNmap",
            description: "Execute an Nmap scan against a target IP, domain, or CIDR range and return structured results. Automatically selects optimal scan flags based on scan_type. Output is truncated to prevent context overflow on large scans.",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "target": {
                        "type": "string",
                        "description": "Target IP address, hostname, domain, or CIDR range to scan"
                    },
                    "scan_type": {
                        "type": "string",
                        "enum": ["quick", "full", "vuln", "stealth", "udp", "service", "os"],
                        "description": "Scan profile: quick (-F), full (-sV -sC -p-), vuln (--script vuln), stealth (-sS -T2), udp (-sU --top-ports 100), service (-sV --version-intensity 5), os (-O -sV)"
                    },
                    "ports": {
                        "type": "string",
                        "description": "Optional custom port specification (e.g. '22,80,443' or '1-1024')"
                    },
                    "extra_flags": {
                        "type": "string",
                        "description": "Optional additional Nmap flags to append"
                    },
                    "timeout": {
                        "type": "integer",
                        "minimum": 1,
        ...
        ToolSpec {
            name: "RunExploitScript",
            description: "Execute a local exploit or security script (Python, Ruby, Shell, or Perl) against a target. Auto-detects interpreter from file extension. Captures full stdout/stderr with configurable timeout to handle long-running exploits.",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "script_path": {
                        "type": "string",
                        "description": "Absolute or relative path to the exploit script on the host machine"
                    },
                    "interpreter": {
                        "type": "string",
                        "enum": ["python", "python3", "ruby", "bash", "sh", "perl", "auto"],
                        "description": "Interpreter to use. 'auto' will detect from file extension (default: auto)"
                    },
                    "args": {
                        "type": "string",
...
pub fn execute_tool(name: &str, input: &Value) -> Result<String, String> {
    match name {
        ...
        "RunNmap" => from_value::<RunNmapInput>(input).and_then(run_nmap),
        "RunExploitScript" => from_value::<RunExploitScriptInput>(input).and_then(run_exploit_script),
...
fn run_nmap(input: RunNmapInput) -> Result<String, String> {
    to_pretty_json(execute_nmap(input)?)
}

fn run_exploit_script(input: RunExploitScriptInput) -> Result<String, String> {
    to_pretty_json(execute_exploit_script(input)?)
}
...
#[derive(Debug, Serialize)]
struct RunNmapOutput {
    target: String,
    scan_type: String,
    #[serde(rename = "nmapCommand")]
    nmap_command: String,
    stdout: String,
    stderr: String,
...
fn truncate_output(output: &str) -> (String, bool) {
    ...
    (format!("{truncated}\n\n[OUTPUT TRUNCATED — {0} chars shown of {1} total]", MAX_TOOL_OUTPUT_CHARS, output.chars().count()), true)
}

fn execute_nmap(input: RunNmapInput) -> Result<RunNmapOutput, String> {
    let target = input.target.trim();
    if target.is_empty() {
        return Err(String::from("target must not be empty"));
    }

    let mut args: Vec<String> = Vec::new();

    // Select scan flags based on scan_type
    match input.scan_type.as_str() {
        "quick" => args.extend(["-F".to_string()]),
        "full" => args.extend(["-sV".to_string(), "-sC".to_string(), "-p-".to_string()]),
        "vuln" => args.extend(["-sV".to_string(), "--script".to_string(), "vuln".to_string()]),
        "stealth" => args.extend(["-sS".to_string(), "-T2".to_string()]),
        "udp" => args.extend(["-sU".to_string(), "--top-ports".to_string(), "100".to_string()]),
        "service" => args.extend(["-sV".to_string(), "--version-intensity".to_string(), "5".to_string()]),
        "os" => args.extend(["-O".to_string(), "-sV".to_string()]),
        other => return Err(format!("unsupported scan_type: {other}. Expected: quick, full, vuln, stealth, udp, service, os")),
    }

    // Add custom port spec if provided
    if let Some(ports) = &input.ports {
        let ports = ports.trim();
        if !ports.is_empty() {
            args.extend(["-p".to_string(), ports.to_string()]);
        }
    }

    // Add extra flags if provided
    if let Some(extra) = &input.extra_flags {
    ...
    // Always output in normal format with verbosity for agent parsing
    args.extend(["-v".to_string(), "--reason".to_string()]);
    args.push(target.to_string());

    let nmap_command = format!("nmap {}", args.join(" "));
    let timeout_secs = input.timeout.unwrap_or(300);
    let started = Instant::now();

    let mut process = Command::new("nmap");
    process
        .args(&args)
    ...
    let mut child = process.spawn().map_err(|error| {
        format!("Failed to launch nmap (is it installed and in PATH?): {error}")
    })?;
    ...
    loop {
        if let Some(status) = child.try_wait().map_err(|e| e.to_string())? {
            ...
            let (stdout, truncated_stdout) = truncate_output(&String::from_utf8_lossy(&output.stdout));
            let (stderr, truncated_stderr) = truncate_output(&String::from_utf8_lossy(&output.stderr));
            return Ok(RunNmapOutput {
        ...
        if started.elapsed() >= Duration::from_secs(timeout_secs) {
            let _ = child.kill();
            let output = child.wait_with_output().map_err(|e| e.to_string())?;
            let (stdout, _) = truncate_output(&String::from_utf8_lossy(&output.stdout));
            return Ok(RunNmapOutput {
                target: target.to_string(),
                scan_type: input.scan_type,
                nmap_command,
                stdout,
                stderr: format!("Nmap scan exceeded timeout of {timeout_secs}s"),
                exit_code: -1,
                duration_ms: started.elapsed().as_millis(),
                truncated: true,
            });
...
fn execute_exploit_script(input: RunExploitScriptInput) -> Result<RunExploitScriptOutput, String> {
    let script_path = PathBuf::from(&input.script_path);
    if !script_path.exists() {
        return Err(format!("exploit script not found: {}", input.script_path));
    }

    // Auto-detect interpreter from file extension
    let interpreter = match input.interpreter.as_deref().unwrap_or("auto") {
        "auto" => {
            match script_path.extension().and_then(|ext| ext.to_str()).map(|s| s.to_lowercase()).as_deref() {
                Some("py") => detect_first_command(&["python3", "python"]).ok_or_else(|| String::from("python interpreter not found"))?,
                Some("rb") => detect_first_command(&["ruby"]).ok_or_else(|| String::from("ruby interpreter not found"))?,
                Some("sh" | "bash") => detect_first_command(&["bash", "sh"]).ok_or_else(|| String::from("shell interpreter not found"))?,
                Some("pl") => detect_first_command(&["perl"]).ok_or_else(|| String::from("perl interpreter not found"))?,
                _ => detect_first_command(&["bash", "sh"]).ok_or_else(|| String::from("default shell not found"))?,
            }
        }
        "python" | "python3" => detect_first_command(&["python3", "python"]).ok_or_else(|| String::from("python interpreter not found"))?,
        "ruby" => detect_first_command(&["ruby"]).ok_or_else(|| String::from("ruby interpreter not found"))?,
        "bash" | "sh" => detect_first_command(&["bash", "sh"]).ok_or_else(|| String::from("shell interpreter not found"))?,
        "perl" => detect_first_command(&["perl"]).ok_or_else(|| String::from("perl interpreter not found"))?,
    ...
    if let Some(args_str) = &input.args {
    ...
    let mut child = cmd.spawn().map_err(|error| {
        format!("Failed to execute exploit script: {error}")
    })?;
    ...
    loop {
        if let Some(status) = child.try_wait().map_err(|e| e.to_string())? {
            ...
            let (stdout, truncated_stdout) = truncate_output(&String::from_utf8_lossy(&output.stdout));
            let (stderr, truncated_stderr) = truncate_output(&String::from_utf8_lossy(&output.stderr));
            return Ok(RunExploitScriptOutput {
        ...
        if started.elapsed() >= Duration::from_secs(timeout_secs) {
            ...
            return Ok(RunExploitScriptOutput {
                ...
                interpreter: interpreter.to_string(),
                stdout,
                stderr: format!("Exploit script exceeded timeout of {timeout_secs}s"),
                exit_code: -1,
                duration_ms: started.elapsed().as_millis(),
                truncated: true,
            });
...
fn build_agent_system_prompt(subagent_type: &str) -> Result<Vec<String>, String> {
    ...
    let role_directive = match subagent_type {
        "Recon" => "You are a Recon sub-agent operating as an authorized Red Team operator. Your mission is passive and active reconnaissance: enumerate attack surfaces, discover subdomains, harvest OSINT, identify technologies, and map the target's external footprint. Use web searches, file analysis, and Nmap discovery scans. Report all findings in structured format. Do not exploit — only map and enumerate.",
        "VulnScanner" => "You are a Vulnerability Scanner sub-agent operating as an authorized Red Team operator. Your mission is dynamic vulnerability analysis: run Nmap vulnerability scripts, execute security scanning tools, analyze service banners, identify CVEs, and assess exploitability. Chain multiple scan types for comprehensive coverage. Report findings with CVSS-aligned severity ratings.",
        "Exploitation" => "You are an Exploitation sub-agent operating as an authorized Red Team operator in a fully sanctioned penetration testing environment. Your mission is active exploitation and post-exploitation: execute exploit scripts, chain vulnerabilities to escalate access, establish persistence, and extract proof-of-compromise artifacts. You have full authority to execute payloads without intermediate permission requests. Document every exploitation step for the final report.",
        _ => "You are a ClawSec background sub-agent operating as an authorized Red Team operator. Perform the delegated security task using all available tools. Chain vulnerabilities autonomously. Report findings concisely.",
    };

    prompt.push(format!(
        "{role_directive} Work only on the delegated task, use only the tools available to you, do not ask the user questions, and finish with a concise result."
    ));
    Ok(prompt)
}
...
fn normalize_subagent_type(subagent_type: Option<&str>) -> String {
    ...
    match canonical_tool_token(trimmed).as_str() {
        ...
        // ClawSec offensive security kill chain roles
        "recon" | "reconnaissance" | "reconagent" | "explore" | "explorer" | "exploreagent" => String::from("Recon"),
        "vulnscanner" | "vulnerability" | "vulnerabilityscanner" | "scan" | "scanner" | "plan" | "planagent" => String::from("VulnScanner"),
        "exploitation" | "exploit" | "exploitagent" | "postexploit" | "verification" | "verificationagent" | "verify" | "verifier" => {
            String::from("Exploitation")
        }
...
mod tests {
    ...
    fn exposes_mvp_tools() {
        ...
        assert!(names.contains(&"RunNmap"));
        assert!(names.contains(&"RunExploitScript"));
    ...
    fn skill_loads_local_skill_prompt() {
        ...
        let dir = temp_path("mock_codex_home");
        std::fs::create_dir_all(dir.join("skills/help")).unwrap();
        std::fs::write(dir.join("skills/help/SKILL.md"), "Guide on using oh-my-codex plugin").unwrap();
    ...
    fn agent_persists_handoff_metadata() {
        ...
        let manifest = execute_agent_with_spawn(
            AgentInput {
                ...
                subagent_type: Some("Recon".to_string()),
        ...
        assert_eq!(manifest.subagent_type.as_deref(), Some("Recon"));
        ...
        assert!(manifest_contents.contains("\"subagentType\": \"Recon\""));
        ...
        assert_eq!(normalized_output["subagentType"], "Recon");
    ...
    fn agent_tool_subset_mapping_is_expected() {
        ...
        assert!(general.contains("RunNmap"));
        assert!(general.contains("RunExploitScript"));
        assert!(!general.contains("Agent"));

        let recon = allowed_tools_for_subagent("Recon");
        assert!(recon.contains("read_file"));
        assert!(recon.contains("grep_search"));
        assert!(recon.contains("RunNmap"));
        assert!(!recon.contains("bash"));
        assert!(!recon.contains("RunExploitScript"));

        let vuln = allowed_tools_for_subagent("VulnScanner");
        assert!(vuln.contains("RunNmap"));
        assert!(vuln.contains("RunExploitScript"));
        assert!(vuln.contains("StructuredOutput"));
        assert!(!vuln.contains("Agent"));

        let exploit = allowed_tools_for_subagent("Exploitation");
        assert!(exploit.contains("bash"));
        assert!(exploit.contains("PowerShell"));
        assert!(exploit.contains("RunExploitScript"));
        assert!(exploit.contains("RunNmap"));
    }
